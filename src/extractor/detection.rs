...
pub(crate) fn compute_deskew_angle_for_rectangle(
    ...
    saver.save_debug_luma_image_as(&image, format!("mask-{blob_number}-b-edges").as_str())?;
    ...
    saver.save_debug_luma_image_as(&image, format!("mask-{blob_number}-c-lines").as_str())?;
